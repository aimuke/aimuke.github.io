...
3. 可伸缩性
   ...
   业务量大时除了提前预估服务器外，在极端的情况下，可以实现动态的服务器扩容。
...
**分布式与集群**
分布式是侧重于不同的业务分布在不同的地方，集群侧重于几台服务器集中在一起实现同一业务。分布式中的每一个节点，都可以做集群，而集群并不一定就是分布式的。

# References

- [分布式与集群的区别](https://www.zhihu.com/question/20004877)

- [阿里P8架构师谈：分布式、集群、负载均衡、分布式数据一致性的区别与关联](http://youzhixueyuan.com/the-difference-between-distributed-cluster-and-load-balancing.html)
