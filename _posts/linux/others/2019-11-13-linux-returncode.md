---
title: "linux命令执行返回值（附错误对照表）"
tags: [linux, windows]
--- 

在 Linux 下，不管你是启动一个桌面程序也好，还是在控制台下运行命令，所有的程序在结束时，都会返回一个数字值，这个值叫做 `返回值` ，或者称为 `错误号` ( `Error Number` )。
...
先随便执行个命令，比如像上面的 `ls` 某些文件，然后通过 `echo $?`，打印 `$?` 的值。我们发现返回值是 `0`，这是什么意思呢？

- `0`，就代表程序执行成功了。
- 非 `0`，就表示上一个命令在执行的过程中出错了。
...
- [原文 - linux命令执行返回值（附错误对照表）](http://blog.sina.com.cn/s/blog_6739945f0100zt4b.html)
